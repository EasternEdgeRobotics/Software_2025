...
public:
  SimulationBotControl() : Node("SimulationBotControl")
  {
    ...
    auto thrust_values_subscriber_callback =
      [this](waterwitch_pkg::msg::WaterwitchThrustValues::UniquePtr thrust_values_msg) -> void {
        for (int thruster_index = 0; thruster_index < 6; thruster_index++) {
    ...
    thrust_values_subscriber =
      this->create_subscription<waterwitch_pkg::msg::WaterwitchThrustValues>(
        "thrust_values", 10, thrust_values_subscriber_callback);
  }

private:
  rclcpp::Subscription<waterwitch_pkg::msg::WaterwitchThrustValues>::SharedPtr thrust_values_subscriber;
